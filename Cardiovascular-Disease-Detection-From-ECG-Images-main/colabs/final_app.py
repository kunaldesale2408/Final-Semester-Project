...
if uploaded_file is not None:
  ...
  image1=imread(r'E:\Cardiovascular-Disease-Detection-From-ECG-Images-main\Cardiovascular-Disease-Detection-From-ECG-Images-main\ECG_IMAGES_DATASET\ECG Images of Patient that have History of MI (172x12=2064)\PMI(1).jpg')
  image1 = color.rgb2gray(image1)
  image1=resize(image1,(1572,2213))

  image2=imread(r'E:\Cardiovascular-Disease-Detection-From-ECG-Images-main\Cardiovascular-Disease-Detection-From-ECG-Images-main\ECG_IMAGES_DATASET\ECG Images of Patient that have abnormal heartbeat (233x12=2796)\\HB(6).jpg')
  image2 = color.rgb2gray(image2)
  image2=resize(image2,(1572,2213))

  image3=imread(r'E:\Cardiovascular-Disease-Detection-From-ECG-Images-main\Cardiovascular-Disease-Detection-From-ECG-Images-main\ECG_IMAGES_DATASET\Normal Person ECG Images (284x12=3408)\\Normal(1).jpg')
  image3 = color.rgb2gray(image3)
  image3=resize(image2,(1572,2213))

  image4=imread(r'E:\Cardiovascular-Disease-Detection-From-ECG-Images-main\Cardiovascular-Disease-Detection-From-ECG-Images-main\ECG_IMAGES_DATASET\ECG Images of Myocardial Infarction Patients (240x12=2880)\\MI(1).jpg')
  image4 = color.rgb2gray(image4)
  image4=resize(image2,(1572,2213))

  similarity_score=max(structural_similarity(image_gray,image1,data_range=1.0),structural_similarity(image_gray,image2,data_range=1.0),structural_similarity(image_gray,image3,data_range=1.0),structural_similarity(image_gray,image4,data_range=1.0))
  ...
  if similarity_score > 0.70:
    ...
    dataframes = []
    test_final=pd.read_csv(r'E:\Cardiovascular-Disease-Detection-From-ECG-Images-main\Cardiovascular-Disease-Detection-From-ECG-Images-main\colabs\scaled_1DLead_1.csv')
    test_final_clean = test_final.dropna()
    location= r'E:\Cardiovascular-Disease-Detection-From-ECG-Images-main\Cardiovascular-Disease-Detection-From-ECG-Images-main\colabs'
    # for files in natsorted(os.listdir(location)):
    #   if files.endswith(".csv"):
    #     if files!='Scaled_1DLead_1.csv':
    #         df=pd.read_csv(r'E:\Cardiovascular-Disease-Detection-From-ECG-Images-main\Cardiovascular-Disease-Detection-From-ECG-Images-main\colabs\{}'.format(files))
    #         test_final=pd.concat([test_final,df],axis=1,ignore_index=True)
    for file in natsorted(os.listdir(location)):
      if file.endswith(".csv"):
        df = pd.read_csv(os.path.join(location, file))
        dataframes.append(df)
    #test_final = pd.concat(dataframes, axis=1, ignore_index=True)
    test_final = pd.read_csv(r'E:\Cardiovascular-Disease-Detection-From-ECG-Images-main\Cardiovascular-Disease-Detection-From-ECG-Images-main\colabs\Scaled_1DLead_1.csv')
    test_final_clean = test_final.dropna()
    ...
    st.write(test_final_clean)

    """#### **PASS TO ML MODEL FOR PREDICTION**"""
    loaded_model = joblib.load(r'E:\Cardiovascular-Disease-Detection-From-ECG-Images-main\Cardiovascular-Disease-Detection-From-ECG-Images-main\colabs\Heart_Disease_Prediction_using_ECG.pkl(4)')
    result = loaded_model.predict(test_final_clean)
    if result[0] == 2:
